...
extension YFAPIURLBuilder {
    ...
    nonisolated struct WebSocketBuilder: Sendable {
        ...
        /// - Returns: 새로운 빌더 인스턴스
        func version(_ version: Int) -> WebSocketBuilder {
        ...
        ///   - value: 파라미터 값
        /// - Returns: 새로운 빌더 인스턴스
        func parameter(_ key: String, _ value: String) -> WebSocketBuilder {
            var newParams = parameters
            newParams[key] = value
            return WebSocketBuilder(session: session, parameters: newParams)
        }
...
extension YFAPIURLBuilder.WebSocketBuilder {
    ...
    /// - Returns: 구성된 WebSocket 클라이언트
    /// - Throws: URL 구성 실패 시
    func defaultClient(verbose: Bool = false) async throws -> YFWebSocketClient {
    ...
    /// - Throws: URL 구성 실패 시
    func debugClient() async throws -> YFWebSocketClient {
