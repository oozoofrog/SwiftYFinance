...
extension YFAPIURLBuilder {
    ...
    /// Fundamentals API 전용 빌더 (내부 구현 타입)
    /// nonisolated: 순수 값 타입 빌더 — actor isolation 불필요
    nonisolated struct FundamentalsBuilder: Sendable {
        ...
        /// - Returns: 새로운 빌더 인스턴스
        func symbol(_ symbol: String) -> FundamentalsBuilder {
        ...
        /// - Returns: 새로운 빌더 인스턴스
        func types(_ types: [String]) -> FundamentalsBuilder {
            var newParams = parameters
        ...
        ///   - endPeriod: 종료 기간
        /// - Returns: 새로운 빌더 인스턴스
        func period(from startPeriod: String, to endPeriod: String) -> FundamentalsBuilder {
            var newParams = parameters
        ...
        /// - Returns: 새로운 빌더 인스턴스
        func merge(_ merge: Bool) -> FundamentalsBuilder {
            var newParams = parameters
        ...
        ///   - value: 파라미터 값
        /// - Returns: 새로운 빌더 인스턴스
        func parameter(_ key: String, _ value: String) -> FundamentalsBuilder {
            var newParams = parameters
            newParams[key] = value
            return FundamentalsBuilder(session: session, symbol: symbol, parameters: newParams)
        }
