...
extension YFAPIURLBuilder {
    ...
    struct DomainBuilder: Sendable {
        ...
        /// - Returns: 새로운 빌더 인스턴스
        func sector(_ sector: YFSector) -> DomainBuilder {
        ...
        /// - Returns: 새로운 빌더 인스턴스
        func sectorKey(_ key: String) -> DomainBuilder {
            return DomainBuilder(
                session: session,
                domainType: .sector,
                domainKey: key,
        ...
        /// - Returns: 새로운 빌더 인스턴스
        func industry(_ industry: String) -> DomainBuilder {
            return DomainBuilder(
                session: session,
                domainType: .industry,
                domainKey: industry,
        ...
        /// - Returns: 새로운 빌더 인스턴스
        func market(_ market: YFMarket) -> DomainBuilder {
            return DomainBuilder(
                session: session,
                domainType: .market,
        ...
        /// - Returns: 새로운 빌더 인스턴스
        func marketKey(_ key: String) -> DomainBuilder {
            return DomainBuilder(
                session: session,
                domainType: .market,
                domainKey: key,
        ...
        /// - Returns: 새로운 빌더 인스턴스
        func count(_ count: Int) -> DomainBuilder {
        ...
        /// - Returns: 새로운 빌더 인스턴스
        func offset(_ offset: Int) -> DomainBuilder {
            var newParams = parameters
        ...
        /// - Returns: 새로운 빌더 인스턴스
        func sortField(_ field: String) -> DomainBuilder {
            var newParams = parameters
        ...
        /// - Returns: 새로운 빌더 인스턴스
        func sortOrder(ascending: Bool = false) -> DomainBuilder {
            var newParams = parameters
        ...
        ///   - value: 파라미터 값
        /// - Returns: 새로운 빌더 인스턴스
        func parameter(_ key: String, _ value: String) -> DomainBuilder {
            var newParams = parameters
            newParams[key] = value
            return DomainBuilder(session: session, domainType: domainType, domainKey: domainKey, parameters: newParams)
        }
...
extension YFAPIURLBuilder.DomainBuilder {
    ...
    /// 기술 섹터의 주요 산업들 조회
    /// - Returns: 새로운 빌더 인스턴스
    func technologySector() -> YFAPIURLBuilder.DomainBuilder {
        return sector(.technology)
    }

    /// 의료 섹터의 주요 산업들 조회
    /// - Returns: 새로운 빌더 인스턴스
    func healthcareSector() -> YFAPIURLBuilder.DomainBuilder {
        return sector(.healthcare)
    }

    /// 금융 서비스 섹터의 주요 산업들 조회
    /// - Returns: 새로운 빌더 인스턴스
    func financialServicesSector() -> YFAPIURLBuilder.DomainBuilder {
        return sector(.financialServices)
    }

    /// 미국 시장 정보 조회
    /// - Returns: 새로운 빌더 인스턴스
    func usMarket() -> YFAPIURLBuilder.DomainBuilder {
        return market(.us)
    }

    /// 반도체 산업 정보 조회
    /// - Returns: 새로운 빌더 인스턴스
    func semiconductorIndustry() -> YFAPIURLBuilder.DomainBuilder {
        return industry("semiconductors")
    }

    /// 소프트웨어 인프라 산업 정보 조회
    /// - Returns: 새로운 빌더 인스턴스
    func softwareInfrastructureIndustry() -> YFAPIURLBuilder.DomainBuilder {
