...
extension YFAPIURLBuilder {
    ...
    /// Search API 전용 빌더 (내부 구현 타입)
    /// nonisolated: 순수 값 타입 빌더 — actor isolation 불필요
    nonisolated struct SearchBuilder: Sendable {
        ...
        /// - Parameter query: 검색어
        /// - Returns: 새로운 빌더 인스턴스
        func query(_ query: String) -> SearchBuilder {
        ...
        /// - Returns: 새로운 빌더 인스턴스
        func quotesCount(_ count: Int) -> SearchBuilder {
            var newParams = parameters
        ...
        /// - Returns: 새로운 빌더 인스턴스
        func newsCount(_ count: Int) -> SearchBuilder {
            var newParams = parameters
        ...
        /// - Returns: 새로운 빌더 인스턴스
        func fuzzyQuery(_ enabled: Bool) -> SearchBuilder {
            var newParams = parameters
        ...
        ///   - value: 파라미터 값
        /// - Returns: 새로운 빌더 인스턴스
        func parameter(_ key: String, _ value: String) -> SearchBuilder {
            var newParams = parameters
            newParams[key] = value
            return SearchBuilder(session: session, parameters: newParams)
        }
