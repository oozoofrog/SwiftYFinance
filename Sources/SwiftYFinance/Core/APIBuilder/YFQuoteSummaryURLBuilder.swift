...
extension YFAPIURLBuilder {
    ...
    /// Quote Summary API 전용 빌더 (내부 구현 타입)
    ///
    ...
    struct QuoteSummaryBuilder: Sendable {
        ...
        /// - Returns: 새로운 빌더 인스턴스
        func symbol(_ symbol: String) -> QuoteSummaryBuilder {
        ...
        /// - Returns: 새로운 빌더 인스턴스
        func modules(_ modules: [YFQuoteSummaryModule]) -> QuoteSummaryBuilder {
        ...
        /// - Returns: 새로운 빌더 인스턴스
        func module(_ module: YFQuoteSummaryModule) -> QuoteSummaryBuilder {
        ...
        /// - Returns: 새로운 빌더 인스턴스
        func formatted(_ formatted: Bool) -> QuoteSummaryBuilder {
            var newParams = parameters
        ...
        /// - Returns: 새로운 빌더 인스턴스
        func corsDomain(_ domain: String) -> QuoteSummaryBuilder {
            var newParams = parameters
        ...
        /// 필수 정보 모듈들로 설정
        /// - Returns: 새로운 빌더 인스턴스
        func essential() -> QuoteSummaryBuilder {
            return modules(YFQuoteSummaryModule.essential)
        }

        /// 종합 분석용 모듈들로 설정
        /// - Returns: 새로운 빌더 인스턴스
        func comprehensive() -> QuoteSummaryBuilder {
        ...
        /// - Returns: 새로운 빌더 인스턴스
        func financials(quarterly: Bool = false) -> QuoteSummaryBuilder {
        ...
        ///   - value: 파라미터 값
        /// - Returns: 새로운 빌더 인스턴스
        func parameter(_ key: String, _ value: String) -> QuoteSummaryBuilder {
            var newParams = parameters
            newParams[key] = value
            return QuoteSummaryBuilder(session: session, symbol: symbol, parameters: newParams)
        }
